...
export default function ProjectsPage() {
    ...
    (async () => {
      ...
      try {
        ...
        const mapImage = (img) => {
          ...
          return supabase.storage.from("assets").getPublicUrl(img).data
            .publicUrl;
        };
        const withUrls = (data || []).map((row) => ({
          ...row,
          image: mapImage(
            row.image_path ?? row.main_image_path ?? row.image ?? "",
          ),
        }));
        if (mounted)
          setOngoingProjects(Array.isArray(withUrls) ? withUrls : []);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (mounted)
          setCompletedProjects(Array.isArray(withUrls) ? withUrls : []);
      } catch (err) {
  ...
  async function fetchProjectStudents(projectId) {
    ...
    try {
      const [phdLinksRes, mastersLinksRes, internsLinksRes] = await Promise.all(
        [
          supabase
            .from("project_phd_students")
            .select("*")
            .eq("project_id", projectId),
          supabase
            .from("project_masters_students")
            .select("*")
            .eq("project_id", projectId),
          supabase
            .from("project_research_interns")
            .select("*")
            .eq("project_id", projectId),
        ],
      );
      ...
      const phdIds = (phdLinksRes.data || [])
        .map((r) => r.student_id)
        .filter(Boolean);
      const mastersIds = (mastersLinksRes.data || [])
        .map((r) => r.student_id)
        .filter(Boolean);
      const internsIds = (internsLinksRes.data || [])
        .map((r) => r.intern_id || r.student_id)
        .filter(Boolean);

      const [phdRes, mastersRes, internsRes] = await Promise.all([
        phdIds.length
          ? supabase.from("phd_students").select("*").in("id", phdIds)
          : { data: [], error: null },
        mastersIds.length
          ? supabase.from("masters_students").select("*").in("id", mastersIds)
          : { data: [], error: null },
        internsIds.length
          ? supabase.from("research_interns").select("*").in("id", internsIds)
          : { data: [], error: null },
      ]);
      ...
      const mapImage = (img) => {
        ...
        if (img.startsWith && (img.startsWith("http") || img.startsWith("/")))
          return img;
        try {
          const res = supabase.storage.from("assets").getPublicUrl(img);
      ...
      const phdStudents = (phdRes.data || []).map((s) => ({
        id: s.id,
        name: s.name || s.full_name || s.email || "PhD",
        designation: "PhD",
        image: mapImage(s.image || s.image_path || ""),
      }));
      const mastersStudents = (mastersRes.data || []).map((s) => ({
        id: s.id,
        name: s.name || s.full_name || s.email || "Masters",
        designation: "Masters",
        image: mapImage(s.image || s.image_path || ""),
      }));
      const internsStudents = (internsRes.data || []).map((s) => ({
        id: s.id,
        name: s.name || s.full_name || s.email || "Intern",
        designation: "Intern",
        image: mapImage(s.image || s.image_path || ""),
      }));
  ...
  function renderFunding(project) {
    ...
    if (
      ...
      return list(project.funding_sources);
    if (
      project.fundingSources &&
      Array.isArray(project.fundingSources) &&
      project.fundingSources.length
    )
      return list(project.fundingSources);
    if (
      project.funding &&
      Array.isArray(project.funding) &&
      project.funding.length
    )
      return list(project.funding);

    // fallback: comma-separated string
    if (
      project.funding_sources &&
      typeof project.funding_sources === "string" &&
      project.funding_sources.trim()
    ) {
      const parts = project.funding_sources
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    if (
      project.fundingSources &&
      typeof project.fundingSources === "string" &&
      project.fundingSources.trim()
    ) {
      const parts = project.fundingSources
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      if (parts.length) return list(parts);
    ...
    if (
      ...
      project.funding.trim()
    ) {
      const parts = project.funding
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
      if (parts.length) return list(parts);
  ...
  return (
    <div
      ...
      style={{ padding: 20, backgroundColor: "#011317", minHeight: "80vh" }}
    >
      ...
      {activeTab === "completed" &&
        (loadingCompleted ? (
          ...
          <p style={{ color: "salmon" }}>
            Error loading completed projects: {completedError}
          </p>
        ) : filteredCompleted.length > 0 ? (
          <>
            <h2 style={{ ...headingfont, color: "white" }}>
              Completed Projects
            </h2>
                ...
                <div key={project.id} className="project-row">
                  ...
                  <div className="project-right">
                    ...
                    <div className="project-members">
                      ...
                      {loadingProjectStudents[project.id] && (
                        <span style={{ color: "#c7efe7" }}>Loading...</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {activeTab === "ongoing" &&
        (loadingOngoing ? (
          ...
          <p style={{ color: "salmon" }}>
            Error loading ongoing projects: {ongoingError}
          </p>
        ) : filteredOngoing.length > 0 ? (
          <>
            <h2 style={{ ...headingfont, color: "white", marginTop: 32 }}>
              Ongoing Projects
            </h2>
            <div className="projects-list">

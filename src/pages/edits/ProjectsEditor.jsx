  ...
  const loadProjects = useCallback(async (table) => {
    ...
    try {
      ...
      const mapped = (data || []).map((r) => ({
        ...r,
        mainImageUrl: r?.main_image_path
          ? supabase.storage.from("assets").getPublicUrl(r.main_image_path).data
              .publicUrl
          : "",
      }));
  ...
  async function saveProject(idx) {
    ...
    try {
      const payload = {
        name: p.name || "",
        content: p.content || "",
        funding_sources: Array.isArray(p.funding_sources)
          ? p.funding_sources
          : [],
      ...
      <div style={{ flex: 1 }}>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div>
              <button
                onClick={() => setCurrentTable("ongoing_projects")}
                style={{
                  marginRight: 6,
                  background:
                    currentTable === "ongoing_projects" ? "#2563eb" : undefined,
                  color:
                    currentTable === "ongoing_projects" ? "#fff" : undefined,
                }}
              >
                Ongoing
              </button>
              <button
                onClick={() => setCurrentTable("completed_projects")}
                style={{
                  marginRight: 8,
                  background:
                    currentTable === "completed_projects"
                      ...
                      : undefined,
                  color:
                    currentTable === "completed_projects" ? "#fff" : undefined,
                }}
              >
                Completed
              </button>
            </div>
            <button onClick={addEmpty} style={{ marginRight: 8 }}>
              + New
            </button>
              ...
              >
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    />
                    {(proj.funding_sources || []).map((f, fi) => {
                      const funding = fundingParser(f);
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              const arr = [...(proj.funding_sources || [])];
                              const obj = fundingParser(arr[fi]);
                              obj.name = e.target.value;
                              arr[fi] = JSON.stringify(obj);
                              updateField(i, "funding_sources", arr);
                            }}
                          />

                          <input
                            value={funding.link || ""}
                            placeholder="Funding source link"
                            style={{ width: "20%", padding: 8 }}
                            onChange={(e) => {
                              const arr = [...(proj.funding_sources || [])];
                              const obj = fundingParser(arr[fi]);
                    ...
                    <button
                      onClick={() => {
                        const arr = [...(proj.funding_sources || [])];
                        arr.push(
                          JSON.stringify({ name: "", link: "", amount: "" }),
                        );
                        updateField(i, "funding_sources", arr);
                      }}
                      style={{ marginTop: 8 }}
                      ...
                      >
                        ...
                        >
                          ...
                          <input
                            ...
                            accept="image/*"
                            onChange={(e) =>
                              uploadMainImage(e.target.files?.[0], i)
                            }
                          />
                        </div>
                        <button
